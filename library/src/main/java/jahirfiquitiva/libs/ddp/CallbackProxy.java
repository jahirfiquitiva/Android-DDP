...
public class CallbackProxy implements MeteorCallback {
    ...
    public void onDataChanged(final String collectionName, final String documentID,
                              final String updatedValuesJson, final String removedValuesJson) {
        // iterate over all the registered callbacks
        for (final MeteorCallback callback : mCallbacks) {
            // if the callback exists
            if (callback != null) {
                ...
                mUiHandler.post(new Runnable() {
                    ...
                    public void run() {
                        // run the proxied method with the same parameters
                        callback.onDataChanged(collectionName, documentID, updatedValuesJson,
                                               removedValuesJson);
                    }
    ...
    public SubscribeListener forSubscribeListener(final SubscribeListener callback) {
        return new SubscribeListener() {
            ...
            @Override
            public void onSuccess(final String result, final String collection,
                                  final String details, final String message) {
                // if the callback exists
                if (callback != null) {
                    // execute the callback on the main thread
                    mUiHandler.post(new Runnable() {
                        ...
                        public void run() {
                            // run the proxied method with the same parameters
                            callback.onSuccess(result, collection, details, message);
                        }
