...
import jahirfiquitiva.libs.ddp.db.Document;
import jahirfiquitiva.libs.ddp.db.Query;
...
public final class InMemoryQuery implements Query {
    ...
    public Query whereEqual(final String fieldName, final Object fieldValue) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.getField(fieldName) == null ||
                    !entry.getField(fieldName).equals(fieldValue)) {
                iterator.remove();
            }
        }
    ...
    public Document[] find(final int limit) {
        if (limit <= 0) {
            throw new IllegalArgumentException(
                    "The limit (`" + limit + "`) must be greater than `0`");
        }
    ...
    public Document[] find(final int limit, final int offset) {
        if (limit <= 0) {
            throw new IllegalArgumentException(
                    "The limit is `" + limit + "` but it must be greater than `0`");
        }

        if (offset < 0) {
            throw new IllegalArgumentException(
                    "The offset is `" + offset + "` but it must be greater than or equal to `0`");
        }
        ...
        for (int i = 0; i < offset; i++) {
            ...
            // if another entry was expected but there was none
                ...
                throw new IllegalStateException(
                        "Expected `" + numResults + "` entries but there were only `" + i + "`");
            }
        }
        ...
        for (int i = 0; i < numResults; i++) {
            ...
            // if another entry was expected but there was none
                ...
                // return an error
                throw new IllegalStateException(
                        "Expected `" + numResults + "` entries but there were only `" +
                                (offset + i) + "`");
            }
        }
