package jahirfiquitiva.libs.ddp.db.memory;
...
import jahirfiquitiva.libs.ddp.Fields;
import jahirfiquitiva.libs.ddp.Meteor;
...
public final class InMemoryDatabase implements Database {
    ...
    @Override
    public void onDataAdded(final String collectionName, final String documentId,
                            final Fields newValues) {
        if (!mCollections.containsKey(collectionName)) {
            mCollections.put(collectionName, new InMemoryCollection(collectionName));
        }

        final InMemoryCollection.DocumentsMap collectionData =
                mCollections.get(collectionName).getDocumentsMap();
    ...
    @Override
    public void onDataChanged(final String collectionName, final String documentId,
                              final Fields updatedValues, final String[] removedValues) {
        if (mCollections.containsKey(collectionName)) {
            final InMemoryCollection.DocumentsMap collectionData =
                    mCollections.get(collectionName).getDocumentsMap();
            ...
            Meteor.log(TAG);
            Meteor.log("  Cannot find document `" + documentId + "` to update in collection `" +
                               collectionName + "`");
    ...
    public void onDataRemoved(final String collectionName, final String documentId) {
        if (mCollections.containsKey(collectionName)) {
            ...
            Meteor.log(TAG);
            Meteor.log("  Cannot find document `" + documentId + "` to delete in collection `" +
                               collectionName + "`");
        }
    }
